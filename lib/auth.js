...
export function StaffAuthProvider({ children }) {
    ...
    const login = async (email, password) => {
        ...
        const isStaffAuthorized = decoded && (allowedRoles.includes(decoded.role) || decoded.is_staff_user || decoded.is_superuser);
        if (!decoded || !isStaffAuthorized) {
            throw new Error('This portal is for staff members and administrators only.');
        }
