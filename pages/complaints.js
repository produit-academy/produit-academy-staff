...
function Complaints() {
    ...
    const handleConfirmResolve = async (e) => {
        ...
        setResolving(true);
        try {
            await apiPatch(`/api/staff/module/support/complaints/${resolveModal.id}/`, {
                status: 'Resolved',
    ...
    return (
        <StaffLayout title="Support Complaints">
                        ...
                        <div style={{
                            ...
                            <div className="card" style={{ width: '100%', maxWidth: '500px', margin: '20px' }} onClick={e => e.stopPropagation()}>
                                <h3 className="section-title" style={{ marginBottom: '14px' }}>
                                    Resolve Complaint - #{resolveModal.id}
                                </h3>
                                <form onSubmit={handleConfirmResolve}>
                                    <div style={{ fontSize: '0.88rem', color: 'var(--text-secondary)', marginBottom: '16px' }}>
                                        Subject: <strong style={{ color: 'var(--text-primary)' }}>{resolveModal.subject}</strong><br/>
                                        Student: {resolveModal.student_name}
                                    </div>
